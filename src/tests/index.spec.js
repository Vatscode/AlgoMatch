const { app, TICKER } = require("../");
const request = require("supertest");

describe("Basic tests", () => {
  it("verify initial balances", async () => {
    let res = await request(app).get("/balance/1").send();
    console.log("Initial balance for user 1:", res.body.balances); // Log balance for user 1
    expect(res.body.balances[TICKER]).toBe(10);

    res = await request(app).get("/balance/2").send();
    console.log("Initial balance for user 2:", res.body.balances); // Log balance for user 2
    expect(res.body.balances[TICKER]).toBe(10);
  });
  ...
  it("Can create orders", async () => {
    ...
    console.log("Orderbook depth:", res.body.depth); // Log orderbook state
  ...
  it("ensures balances are still the same", async () => {
    let res = await request(app).get("/balance/1").send();
    console.log("Balance for user 1 after order:", res.body.balances); // Log balance after order
    expect(res.body.balances[TICKER]).toBe(10);
  });
  ...
  it("Places an order that fills", async () => {
    ...
    console.log("Filled quantity:", res.body.filledQuantity); // Log filled quantity
    expect(res.body.filledQuantity).toBe(2);
  });

  it("Ensures orderbook updates", async () => {
    let res = await request(app).get("/depth").send();
    console.log("Orderbook state after order:", res.body.depth); // Log orderbook state after order
    expect(res.body.depth["1400.9"]?.quantity).toBe(8);
  });

  it("Ensures balances update", async () => {
    let res = await request(app).get("/balance/1").send();
    console.log("Balance for user 1 after order fill:", res.body.balances); // Log balance for user 1
    ...
    res = await request(app).get("/balance/2").send();
    console.log("Balance for user 2 after order fill:", res.body.balances); // Log balance for user 2
